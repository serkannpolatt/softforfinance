...
def Supertrend(data, SENSITIVITY=3, ATR_PERIOD=14):
    ...
    for i in range(1, len(Supertrend)):
        if (Supertrend.loc[i, 'Adj Close'] > Supertrend.loc[i - 1, 'ATRTrailing']) and (
                Supertrend.loc[i - 1, 'Adj Close'] > Supertrend.loc[i - 1, 'ATRTrailing']):
            Supertrend.loc[i, 'ATRTrailing'] = max(Supertrend.loc[i - 1, 'ATRTrailing'],
                                                    Supertrend.loc[i, 'Adj Close'] - Supertrend.loc[i, 'nLoss'])

        elif (Supertrend.loc[i, 'Adj Close'] < Supertrend.loc[i - 1, 'ATRTrailing']) and (
                Supertrend.loc[i - 1, 'Adj Close'] < Supertrend.loc[i - 1, 'ATRTrailing']):
            Supertrend.loc[i, 'ATRTrailing'] = min(Supertrend.loc[i - 1, 'ATRTrailing'],
                                                    Supertrend.loc[i, 'Adj Close'] + Supertrend.loc[i, 'nLoss'])
...
Titles = ['Hisse Adı', 'Kazanma Oranı[%]', 'Sharpe Oranı', 'Ort. Kazanma Oranı [%]', 'Ort Kazanma Süresi',
          'Ort. Kayıp Oranı [%]', 'Ort Kayıp Süresi', 'Giriş Sinyali', 'Çıkış Sinyali']
